...
# CPSC 340 code
from utils import plot_classifier, mode, load_dataset, handle, main, run, mode
import grads
...
def general_influenza_stats(values):
    minn = np.min(values)
    maxx = np.max(values)
    mean = np.mean(values)
    ...
    fifth_quantile = np.quantile(values, 0.05)
    twenty_fifth_quantile = np.quantile(values, 0.25)
    fiftieth_quantile = np.quantile(values, 0.5)
    seventy_fifth_quantile = np.quantile(values, 0.75)
    ninety_fifth_quantile = np.quantile(values, 0.95)
    
    print()
    print("---- 5.1 Part 1 ----")
    print("Min:", minn, "Max:", maxx, "Mean:", mean, "Median:", median, "Mode:", modee)
    print("---- ---------- ----")


    print()
    print("---- 5.1 Part 2 ----")
    print("5%:", fifth_quantile, "25%:", twenty_fifth_quantile, "50%:", fiftieth_quantile, "75%:", seventy_fifth_quantile, "90%:", ninety_fifth_quantile)
    print("---- ---------- ----")

def regions_based_mean_variance(df):
    highest_mean = -np.Infinity
    lowest_mean = np.Infinity
    highest_mean_name = ""
    lowest_mean_name = ""

    highest_var = -np.Infinity
    ...
    for i in df.columns:
        col_mean = df[i].mean()
        col_var = df[i].var()
        # print("Col is", i, "mean :",col_mean, "var :", col_var)
        if col_mean > highest_mean:
            highest_mean = col_mean
            highest_mean_name = i
        if col_mean < lowest_mean:
            lowest_mean = col_mean
            lowest_mean_name = i

        if col_var > highest_var:
            highest_var = col_var
            highest_var_name = i
        if col_var < lowest_var:
            lowest_var = col_var
            lowest_var_name = i

    print()
    print("---- 5.1 Part 3 ----")
    print("Column with the highest mean is", highest_mean_name, "with mean :", highest_mean)
    print("Column with the lowest mean is", lowest_mean_name, "with mean :", lowest_mean)
    print("Column with the highest var is", highest_var_name, "with var :", highest_var)
    print("Column with the lowest var is", lowest_var_name, "with var :", lowest_var) 
    print("---- ---------- ----")
...
def q5_1():
    ...
    general_influenza_stats(X)
    regions_based_mean_variance(df)
