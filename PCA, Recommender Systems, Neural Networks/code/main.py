...
def q3_2():
    ...
    # Standardize the Xtrain and Xvalidate
    X_train_std, mu, sigma = utils.standardize_cols(X_train)
    X_valid_std, mu, sigma = utils.standardize_cols(X_valid, mu, sigma)
    ...
    # Compute training error
    y_hat = model.predict(X_train_std)
    err_train = np.mean(y_hat != y_train)
    print("Training error = ", err_train)

    # Compute validation error
    y_hat = model.predict(X_valid_std)
    err_valid = np.mean(y_hat != y_valid)
    print("Validation error     = ", err_valid)
